...
Page({
  ...
  Publish:function(data){
    ...
    var label=data.detail.value.title
    var rewarded=data.detail.value.money
    var jobRequirements=data.detail.value.jobRequirements
    var startTime=data.detail.value.startTime
    if (!content || !deliveryAddress || !rewarded || !endDate || !startTime || !jobRequirements) {
      wx.showToast({
        title: '不能为空！',
        icon: 'error',
        duration: 1000,
      ...
      wx.request({
      url:'http://43.142.99.39:99/patr-time-job/insert', //必填，其他的都可以不填
      data:{  
        content:content, 
